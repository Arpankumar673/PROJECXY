            ...
            <div className="relative isolate group">
                ...
                <div className="flex flex-col lg:flex-row items-center lg:items-end gap-10">
                    <div className="relative group/avatar">
                        ...
                        <img 
                            src={avatar} 
                            className="w-40 h-40 md:w-56 md:h-56 rounded-[56px] border-[8px] md:border-[12px] border-white shadow-soft relative z-10 hover:scale-105 transition-transform duration-500 object-cover bg-white" 
                            alt="Avatar"
                            onError={(e) => e.target.src = `https://ui-avatars.com/api/?name=${profile?.full_name || "ID"}`}
                        />
                        <button 
                            onClick={() => setIsEditOpen(true)}
                            className="absolute bottom-4 right-4 w-12 h-12 bg-projecxy-blue text-white rounded-2xl shadow-xl flex items-center justify-center border-4 border-white z-20 hover:scale-110 active:scale-95 transition-all"
                        >
                    ...
                    <div className="flex-1 text-center lg:text-left space-y-4 pb-6">
                        <div className="space-y-1">
                            <h1 className="text-4xl md:text-6xl font-black text-projecxy-text tracking-tighter uppercase leading-none">{profile?.full_name || 'Innovator Hub'}</h1>
                        ...
                        <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4">
                            ...
                            <div className="bg-blue-50 text-projecxy-blue px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border border-blue-100">
                                <ShieldCheck className="w-4 h-4" /> Grade: Senior Contributor
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 pb-10">
                        ...
                        <Button 
                          onClick={() => setIsEditOpen(true)}
                          className="h-14 rounded-2xl px-10 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-100 hover:-translate-y-1 transition-all"
                        >
                ...
                <Card className="p-10 lg:col-span-1 space-y-10 rounded-[48px] border-none shadow-soft bg-white group border border-gray-100 transition-all hover:shadow-xl">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-4 group/item">
                           ...
                           <p className="text-sm font-bold text-projecxy-text">Roll No: {profile?.roll_no || 'Pending Assignment'}</p>
                        </div>
                    </div>
                ...
                <Card className="p-10 lg:col-span-2 space-y-12 rounded-[48px] border-none shadow-soft bg-white transition-all hover:shadow-xl">
                    <div className="space-y-8">
                        <div className="flex items-center justify-between">
                            ...
                            <button className="text-[10px] font-black tracking-widest uppercase text-projecxy-blue px-4 py-2 bg-blue-50 border border-blue-100 rounded-full" onClick={() => setIsEditOpen(true)}>Expand Matrix</button>
                        </div>
                        <div className="flex flex-wrap gap-4">
                            {profile?.skills && profile.skills.length > 0 ? profile.skills.map(skill => (
                                <span key={skill} className="px-5 py-2.5 bg-gray-50 border border-gray-100 rounded-2xl text-[11px] font-black uppercase tracking-widest text-projecxy-secondary shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all cursor-default">
                                    {skill}
                                </span>
                            )) : (
                              <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest opacity-50 py-4 px-2">No expertise synchronized. Expand your matrix to begin.</p>
                            )}
                        </div>
                    </div>
                            ...
                            {[1, 2].map(i => (
                                <div key={i} className="p-6 bg-gray-50 rounded-3xl border border-transparent hover:border-blue-100 transition-all group flex flex-col justify-between h-40">
                                    <div className="flex items-start justify-between">
                                        ...
                                        <ExternalLink className="w-4 h-4 text-gray-300 group-hover:text-projecxy-blue transition-colors" />
                                    </div>
                                    <div className="space-y-1">
                                        <h4 className="font-black text-projecxy-text uppercase tracking-tight truncate">Institutional Initiative #{i}</h4>
